...
options_wheels = [{'label': i, 'value': i} for i in df['wheels'].unique()]
options_colors = [{'label': i, 'value': i} for i in df['color'].unique()]

style_ext_div = {'fontFamily': 'helvetica',
                 'fondSize': 18,
                 'text-align': 'center'}

app.layout = html.Div([
    dcc.RadioItems(id='wheels',
                   options=options_wheels,
                   value=1),
...
def callback_b(colors_value):
    return "You cose {}".format(colors_value)
